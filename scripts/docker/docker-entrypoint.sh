...
{
	umask 0002 || exit 2
	sudo chown -Rc "${USER}:${GROUP}" "${FOT_HOME_DIR}" "${FOT_API_DATA_DIR}" "${FOT_API_LOGS_DIR}" "${FOT_API_TMP_DIR}" || exit 2
	find "${FOT_API_DIR}" "${FOT_API_DATA_DIR}" -type d -exec chmod 770 {} + || exit 2
	find "${FOT_API_DIR}" "${FOT_API_DATA_DIR}" -type f -not -path "*/scripts/*" -not -path "*/main.py" -exec chmod 660 {} + || exit 2
	find "${FOT_API_DIR}" "${FOT_API_DATA_DIR}" -type d -exec chmod ug+s {} + || exit 2

...
class DbConfig(BaseConfig):
    ...
    dsn_url: AnyUrl = Field(
        default="postgresql+psycopg://{{cookiecutter.project_abbr}}_user:{{cookiecutter.project_abbr}}_password1@localhost:5432/{{cookiecutter.project_abbr}}_db"
    )
    read_dsn_url: Optional[AnyUrl] = Field(default=None)
    table_prefix: constr(strip_whitespace=True) = Field(
        default="{{cookiecutter.project_abbr}}_", max_length=16
    )
